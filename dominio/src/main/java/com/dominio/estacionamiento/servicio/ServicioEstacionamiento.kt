...
import com.dominio.cobro.modelo.CobroTarifa
...
class ServicioEstacionamiento : Estacionamiento() {
    ...
    fun ingresoUsuarioEstacionamiento(servicioUsuario: ServicioUsuario) {
        ...
        val capacidad = Estacionamiento().consultarCapacidad(servicioUsuario.usuario,
            servicioUsuario.consultarLista())
        val diaDeLaSemana = LocalDateTime.now().dayOfWeek.value
        val fechaIngreso = LocalDateTime.now().toLocalDate()
        val restriccion =
            Estacionamiento().restriccionDeIngreso(servicioUsuario.usuario, diaDeLaSemana)
        servicioUsuario.usuario.fechaIngreso = fechaIngreso

        if (capacidad && !restriccion && servicioUsuario.repositorioUsuario.usuarioExiste(
    ...
    fun salidaDeUsuariosEstacionamiento(
        usuario: Usuario,
        cobroTarifaSalida: CobroTarifa,
    ): Int {
        val fechaSalida = LocalDateTime.now().toLocalDate()
        val registroIngreso = usuario.fechaIngreso
        val duracionServicioEstacionamiento =
            8//Todo operacion para calcular las horas entre dos fechas
